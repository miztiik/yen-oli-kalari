...
**The number clause exists because the rest of the section cannot fail.** "Write in plain language" is advice, and advice catches nothing. "Say what the number means, next to the number" is a check a reader applies to a sentence and gets a yes or a no. On this project the number that most needed it was the speaking pace, which stood at an assumed 150 words a minute until it was measured at 126.7 (Guardrail #10). The number that needs it now is a real-time factor: printed bare it means nothing, because it is only a verdict against a job cap once the reader knows how many runners carry the day. A sentence that prints a factor without the hardware and the shard count is the drift this clause catches.
...
10. **Measured, not estimated.** Any claim about throughput, cost, size or quality carries the hardware it was measured on, the date and the spread. An unmeasured number is labelled an estimate and may not justify a design. When a measurement contradicts the design, the design changes. **The speaking pace was the standing example and is now the standing proof.** It was declared as an assumption at 150 words a minute; measured on the runner on 2026-09-12 it is **126.7**, and every figure derived from it moved by about 16 percent. The guardrail worked exactly as intended - the estimate was labelled, so when the measurement arrived there was no argument about what it superseded. The live example is now the shard budget: a real-time factor compared against a job cap is meaningless without the number of runners carrying the day, and the 0.707 figure that stood for a day was derived for a single runner the pipeline never uses. What gets measured is named in four families: pipeline, model, voice evaluation, and hardware. **One exception, and only one: a counterfactual cost in currency on the Console.** It is computed from measured counts and a rate the operator sets, it prints that rate and where it came from, and it is labelled for what it is - what this run would have cost at a hosted provider's price. **It is never presented as a bill**, because nothing bills us.
...
Four naming rules, added at the owner's request on 2026-09-11 and extended on 2026-09-12. Each earns its place from the reason beside it.
...
- **A name must still be true, and a name that has drifted is renamed in the commit that made it drift.** The three rules above are checked when a name is written; this one is checked when the thing it names changes. The test is a sentence: **can the name be read as a literal claim about what the code does, and is that claim true?** `calculate_audio_budget.py` claims it calculates an audio budget, and it does. `price_audio.py` - the name it carried until 2026-09-12 - claims it prices audio, and it never did: there is no money anywhere in the project, nothing is bought or sold, and the script derives minutes, megabytes and wall-clock hours against two caps. A metaphor that was close enough to wave through on the day it was written gets read literally by every later reader, who then looks for a price and finds a capacity model. Reason: a wrong name is worse than a vague one, because a vague name admits it is uninformative and a wrong one asserts something false with confidence. **Renaming is cheap and near-free to review; leaving it is a debt every future reader services.** Where a rename is genuinely out of scope, section 6's correction-level escalation applies - say so, do not leave it silent.
