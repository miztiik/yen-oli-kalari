...
- Before answering, run the bootstrap ritual in [docs/agents/bootstrap.md](../../docs/agents/bootstrap.md). Guardrail #1 (static-first; no service we run, and every computation in the reader's browser or in CI) is your home turf. A static asset may come from a third party; it is judged on bytes and privacy behaviour, never on hostname.
